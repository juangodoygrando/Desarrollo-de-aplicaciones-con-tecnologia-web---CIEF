...
try{
// VERIFICAR USUARIO DUPLICADO EN TABLA USUARIOS
$stmt = $pdo->prepare("SELECT * FROM usuarios WHERE usuario = :usuario"); 
$stmt->bindParam(':usuario', $usuario, PDO::PARAM_STR);
$stmt->execute();
$usuarioExistente = $stmt->fetch(PDO::FETCH_ASSOC);

// VERIFICAR USUARIO DUPLICADO EN TABLA TEMPORAL
$stmtTemp = $pdo->prepare("SELECT * FROM temporal WHERE usuario = :usuario"); 
$stmtTemp->bindParam(':usuario', $usuario, PDO::PARAM_STR);
$stmtTemp->execute();
$usuarioTemporal = $stmtTemp->fetch(PDO::FETCH_ASSOC);

// Si existe en cualquiera de las dos tablas
if ($usuarioExistente || $usuarioTemporal) {
    $_SESSION['user_repe'] = true;
    header('Location: crear_cuenta.php');
    exit();
} 

// VERIFICAR EMAIL DUPLICADO EN TABLA USUARIOS
$stmt = $pdo->prepare("SELECT * FROM usuarios WHERE email = :email");
$stmt->bindParam(':email', $email, PDO::PARAM_STR);
$stmt->execute();
$emailExistente = $stmt->fetch(PDO::FETCH_ASSOC);

// VERIFICAR EMAIL DUPLICADO EN TABLA TEMPORAL
$stmtTempEmail = $pdo->prepare("SELECT * FROM temporal WHERE email = :email");
$stmtTempEmail->bindParam(':email', $email, PDO::PARAM_STR);
$stmtTempEmail->execute();
$emailTemporal = $stmtTempEmail->fetch(PDO::FETCH_ASSOC);

// Si existe en cualquiera de las dos tablas
if ($emailExistente || $emailTemporal) {
...
<body>
  <div class="container">
    ...
    <a href="http://taskin.byethost4.com/registro.php?token=' . $token . '" class="button">Verificar correo</a>
...
';
$subject = 'Confirma tu registro en Taskin';
...
if (!isset($email_error)) {
  ...
  error_log("Error en insert_temporal.php: " . $e->getMessage());
    $_SESSION['error_cuenta'] = true;
    header('Location: crear_cuenta.php');
    exit();
}
